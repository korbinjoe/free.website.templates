...
const CONFIG = {
  ...
  outputDir: 'metadata/tailwindcss',
  screenshotFileName: 'screenshot.png',
  projectRoot: path.resolve(__dirname, '..'),
  templatePath: 'templates/landing/tailwindcss',
  systemChromePath: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'
};

class TailwindCSSScreenshot {
  constructor() {
    this.browser = null;
    this.context = null;
    this.page = null;
  ...
  async takeTailwindCSSScreenshot() {
    const serverRunning = await this.checkServer();
    if (!serverRunning) {
      console.log('❌ 请先启动TailwindCSS模板的开发服务器');
      console.log('💡 运行命令: cd templates/landing/tailwindcss && npm run dev');
      return false;
    }
  ...
  async run() {
    console.log('🎯 TailwindCSS模板长截图生成器');
    ...
    const success = await this.takeTailwindCSSScreenshot();
    
    if (success) {
      console.log('');
      console.log('🎉 TailwindCSS模板长截图生成完成!');
      console.log('📋 总结:');
...
// 运行脚本
const screenshotTool = new TailwindCSSScreenshot();
